...
pub struct BasicIndex<DocumentId: Debug + Clone + Serialize> {
    dim: usize,
...
impl<DocumentId: Debug + Clone + Copy + Serialize + Ord> BasicIndex<DocumentId> {
    ...
    pub fn search(&self, target: &[f32], limit: usize) -> Vec<(DocumentId, f32)> {
        ...
        for (id, vec, magnitude) in &self.data {
...
mod tests {
    ...
    fn test_basic_serialize_deserialize() {
        ...
        let samples = (0..n)
            ...
            .collect::<Vec<Vec<f32>>>();
        let mut index = BasicIndex::new(dimension);
        for (i, sample) in samples.into_iter().enumerate() {
            index.add_owned(sample.clone(), i);
        }

        let decoded = bincode::serialize(&index).unwrap();
        let new_index: BasicIndex<usize> = bincode::deserialize(&decoded).unwrap();
